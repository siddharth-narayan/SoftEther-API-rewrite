...
use crate::cparser::{
    CParser, CompilationUnitContextAttrs, DeclarationContextAll, DeclarationContextAttrs,
    DeclarationSpecifierContextAttrs, DeclarationSpecifiersContextAttrs, DeclaratorContextAttrs,
    DirectDeclaratorContextAttrs, ExternalDeclarationContextAll, ExternalDeclarationContextAttrs,
    ExternalDeclarationContextExt, InitDeclaratorContextAttrs, InitDeclaratorListContextAttrs,
    TranslationUnitContextAttrs,
};
...
pub fn process_ext_decl_to_func(
    ...
    let pointer = if declator.pointer().is_some() {
        "*"
    } else {
        ""
    };
    ...
    let full_text = format!(
        "{} {}{}",
        return_type.unwrap_or(String::new()),
        pointer,
        function_declaration.get_text()
    );
    Some((name, full_text))
}
